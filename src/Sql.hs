...
insertImageTag schema = Query $ append "insert into " $ append (pack schema) ".image_tag (image_id, tag_id) values (?, ?)"

-- Comments
    -- Comments

insertComment :: String -> Query
insertComment schema = Query $ append "insert into " $ append (pack schema) ".image_comment (id, image_id, author, body, posted_at, deleted) values (?, ?, ?, ?, ?, ?)"
...
insertUserAward schema = Query $ append "insert into " $ append (pack schema) ".user_award (id, user_id, title, label, date) values (?, ?, ?, ?, ?)"

insertUserLink :: String -> Query
insertUserLink schema = Query $ append "insert into " $ append (pack schema) ".user_link (user_id, tag_id, date, state) values (?, ?)"

insertUserFavorite :: String -> Query
insertUserFavorite schema = Query $ append "insert into " $ append (pack schema) ".user_favorite (user_id, image_id) values (?, ?)"
...
insertTagImplication schema = Query $ append "insert into " $ append (pack schema) ".tag_implication (tag_id, implied_tag_id) values (?, ?)"
